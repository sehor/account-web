        ...
        <div class="voucher-container">
          ...
          <div class="header">
            <div class="header-left">
              ...
              <div class="header-item">
                <input type="text" v-model="voucherNumber" style="width: 50px;">
                号
              </div>
            </div>
            <div class="header-right">
              <div class="header-item">
                日期
                <input type="text" v-model="voucherDate">
              </div>
              <div class="header-item">
                附单据
                <input type="text" v-model="attachments" style="width: 50px;">
                张
              </div>
            </div>
          </div>
          ...
          <table>
            ...
            <tr v-for="(entry, index) in entries" :key="index">
              <td :class="{ 'green-bg': index === 0 }" @click="editCell($event, entry, 'summary')">
                 <span>{{ entry.summary}}</span>
                  <input type="text" v-model="entry.summary" class="editable" @blur="hideInput($event,entry,'summary')"
                  @keydown="handleKeydown($event, entry, 'summary')" />
              </td>
              <td class="account" :data-index="index">
                <SearchableSelect class="searchable-select" v-model="entry.account"
                  :options="flattenAccountDataForSelector" @input="updateAccount($event, index)"
                  @keydown="handleKeydown($event, entry, 'account')" />
              </td>
              <td class="right-align bold-text number-cell" :data-index="index" data-field="debit"
                @click="editCell($event, entry, 'debit')">
                <span>{{ formatNumber(entry.debit) }}</span>
                <input type="text" v-model="entry.debit" class="editable" @blur="hideInput($event,entry,'debit')"
                  @keydown="handleKeydown($event, entry, 'debit')" />
              </td>
              <td class="right-align bold-text number-cell" :data-index="index" data-field="credit"
                @click="editCell($event, entry, 'credit')"><span>{{ formatNumber(entry.credit) }}</span>
                <input type="text" v-model="entry.credit" class="editable" @blur="hideInput($event,entry,'credit')"
                  @keydown="handleKeydown($event, entry, 'credit')" />
              </td>
...
export default {
  ...
  computed: {
    totalDebit() {
      return this.entries.reduce((sum, entry) => sum +parseFloat(entry.debit), 0);
    },
    totalCredit() {
      return this.entries.reduce((sum, entry) => sum + parseFloat(entry.credit), 0);
    },
    totalAmountInWords() {
      // 实现金额转大写的逻辑
  ...
  methods: {
    ...
    addRow(index) {
      this.entries.splice(index + 1, 0, { summary: '', account: '', debit: 0, credit: 0 });
    },
    deleteRow(index) {
    ...
    loadEntry(entry) {
      this.voucherType = entry.voucherType;
      this.voucherNumber = entry.voucherNumber;
      this.voucherDate = entry.voucherDate;
    ...
    createNewVoucher() {
      this.voucherType = '记';
      this.voucherNumber = (parseInt(this.voucherNumber) + 1).toString();
      this.voucherDate = new Date().toISOString().split('T')[0];
      this.attachments = '0';
      this.entries = [{ summary: '', account: '', debit: 0, credit: 0 }];
    },
    printVoucher() {
      window.print();
    },
  ...
  mounted() {
    ...
    this.savedEntries.push({
      ...
      attachments: '2',
      entries: [
        { summary: '支付工资', account: '5007001 管理费用_职工薪酬', debit: 50000, credit: 0 },
        { summary: '支付工资', account: '1002001 银行存款_基本户（平安银行）', debit: 0, credit: 50000 }
      ],
      totalAmount: 50000
    });
  }
